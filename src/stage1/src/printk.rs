...
const KERN_SOH: i8 = 1;
const KERN_INFO: i8 = 54;
const KERN_CONT: i8 = 99;
...
impl core::fmt::Write for KernelDebugWriter {
    fn write_str(&mut self, message: &str) -> core::fmt::Result {
        ...
        unsafe {
            // msg + SOH + CONT/INFO + \0
            let ptr = __kmalloc((message.len() + 1 + 1 + 1) as libc::size_t, GFP_KERNEL)
                as *mut libc::c_char;
            if ptr.is_null() {
                ...
                core::ptr::write(ptr, KERN_SOH);

                let next_sym = if self.first_write {
                    self.first_write = false;
                    KERN_INFO
                } else {
                    KERN_CONT
                };
                core::ptr::write(ptr.add(1), next_sym);

                memcpy(
                    ptr.add(2) as *mut libc::c_void,
                    message.as_ptr() as *mut libc::c_void,
                    message.len(),
                );
                core::ptr::write(ptr.add(2 + message.len()), 0);
                printk(ptr);
...
macro_rules! printk {
    ...
    ($fmt:expr, $($arg:tt)*) => ({
        ...
        let mut writer = KernelDebugWriter { first_write: false };
